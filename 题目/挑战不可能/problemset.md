...
zzz最近迷上了写最短路，特别是对于单源最短路。

但是众所周知，Syx超级喜欢出毫无意义的变态题卡人，于是她想让你写一道单源最短路。
...
但是dp就是暴力，zzz只喜欢在dp题打暴力，所以他dp题总是满分。

于是看到这道送分题，zzz用了很多暴力算法（比如LCT，Splay，树链剖分）就轻轻松松AC了本题。
...
如果是 1 $a$ $b$ 表示这是一个操作1，保证$a$!=$b$，意义见题目内容。

如果是 2 $a$ 表示这是一个操作2，意义见题目内容。
...
对于90%的数据，1<=$n$<=100000,1<=$m$<=200000

对于100%的数据，1<=$n$<=300000,1<=$m$<=500000
...
- 1 输出所有植物的质量
- 2 $ \ p \ $ 输出编号为$p$的植物的质量
- 3 $ \ p \ $ $ \ l$ $ \ w_1-w_l \ $在编号为$p$的植物内插入一个含有$l$个孢子的孢子团，每个孢子的质量为长度为$l$的数列(上文中的$w$)。该孢子团变为$m+1$号孢子，其中的孢子变为$k+1$到$k+l$号孢子。
- 4 $ \ b \ $ $ \ l\ $ $ \ w_1-w_l \ $在编号为$b$的孢子团所属的植株中插入一个新的孢子团，$l$与$w$同上。
- 5  删除编号最后的孢子团。
- 6 $ \  b \ $ $ \ w \ $在编号为$b$的孢子团中插入一个质量为$w$的孢子，作为第$m+1$个孢子。
...
$n,m$始终$<=5×10^4$，$0<w<=10^5$，$\Sigma \ k<=10^7$。
...
$l,r,v以及2的结果均>0且在long long范围内(2^{64})$
...
zzz写代码的时候遇到了一个问题，看他的代码解决问题。

#### 题目提交

认真阅读下发文件中的zzz.cpp和zzz.h。

重写zzz.h中的Function函数，然后提交zzz.h作为答案。

#### 题目通过要求

在不超时的情况下帮助zzz在限时内把程序跑完。
