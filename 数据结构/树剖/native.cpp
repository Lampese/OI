...
namespace st{
    struct node{
        ...
        node operator +(const node &x)const{
           return {l,x.r,(sum+x.sum)%p,0,len+x.len};
        }
    }stree[MAXN<<2];
    ...
    void build(int now,int l,int r){
        if(l==r){
            stree[now]={l,l,values[sq[l]]%p,0,1};
            return;
        }
        int mid=(l+r)>>1;
        build(now<<1,l,mid);
    ...
    long long query(int now,int l,int r){
        ...
        return (query(now<<1,l,r)+query(now<<1|1,l,r))%p;
    }
...
inline int child_tree_query(int a){
