...
namespace st{
    ...
    struct node{
        ...
        node operator +(const node &x)const{
           return {l,x.r,len+x.len,sum+x.sum,max(maxx,x.maxx)};
        }
    }stree[MAXN<<2],NP={0,0,0,0,-2147483647};
    void build(int now,int l,int r){
        if(l==r){
            stree[now]={l,l,1,values[sq[l]],values[sq[l]]};
            return;
        }
        int mid=(l+r)>>1;
    ...
    void update(int now,int k,int v){
        if(stree[now].l==stree[now].r){
            stree[now].sum=stree[now].maxx=v;
            return;
        }
...
int main(){
    ...
    for(register int i=1;i<=m;++i){
        cin>>opt;
        read(x),read(y);
        if(opt[0]=='C')st::update(1,dfn[x],y);
