...
namespace st{
    ...
    void update(int now,int k,int v){
        if(stree[now].l==stree[now].r){
            if(stree[now].maxx==2147483647)stree[now].maxx=v;
            else stree[now].maxx=2147483647;
            return;
        }
...
int main(){
    ...
    for(register int i=1;i<=m;++i){
        read(opt),read(x);
        if(opt==0)st::update(1,dfn[x],dfn[x]);
        else{
            auto result=tree_query(x,1);
            if(result.maxx==2147483647)printf("-1\n");
            else printf("%d\n",sq[result.maxx]);
        }
    }
}