...
using namespace std;
#define MAXN 300005
int values[MAXN],head[MAXN],_size[MAXN],father[MAXN],deep[MAXN],top[MAXN],dfn[MAXN],big[MAXN];
...
int main(){
    ...
    st::build(1,1,n);
    for(register int i=1;i<n;++i){
        tree_update(values[i],values[i+1],1);
        tree_update(values[i+1],values[i+1],-1);
    }
    for(register int i=1;i<=n;++i)printf("%lld\n",st::query(1,dfn[i],dfn[i]));
}